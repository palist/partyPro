#if !defined(AFX_SUPERGRIDCTRL_H__917E9031_B8A3_4BEA_9D06_F88EC2637A59__INCLUDED_)
#define AFX_SUPERGRIDCTRL_H__917E9031_B8A3_4BEA_9D06_F88EC2637A59__INCLUDED_
...
//以下两个消息，是supergridctrl类中用到了 
#define WM_USER_LBUTTON_DOWN			WM_USER + 200
#define WM_USER_MOUSE_MOVE				WM_USER + 201
#define WM_USER_LBUTTON_UP				WM_USER + 202
#define WM_USER_DBLCLK					WM_USER + 203
#define WM_USER_SELECTED_TREE_ITEM		WM_USER + 204
#define WM_USER_SELECTED_GRID_ITEM		WM_USER + 205
#define WM_USER_RBUTTON_DOWN			WM_USER + 206
#define WM_USER_CHECK					WM_USER + 207
#define WM_USER_DBUTTON_DOWN			WM_USER + 208


class CItemInfo //should have called this LV_INSERTITEM ..what ever
{
public:
	CItemInfo():m_bCheck(0),m_iImage((const int)-1),m_lParam(NULL),m_clf((COLORREF)-1){m_text_color=RGB(0,0,0);};
	...
	void SetTextColor(long color){m_text_color=color;}
	long GetTextColor(){return m_text_color;}
...
public:
	...
	//选择
	virtual BOOL HitCheck(CTreeItem *hHitItem);
	virtual void OnUpdateCheckState();
private:
...
public:
	...
	void *GetData(int nItem);
	int IsVisual(int nRow);
...
public:
	...
	void InitializeOrderGrid();

	void InitializeHoleGrid();

	void InitializeGCodeGrid();

	void FillGrid(CStringArray &strArray);

	void SelectedItem(int nItem);

//打印
protected:
	int get_layer(CTreeItem *hTreeItem);
	int cal_column_font_count(int nColumn);
	void cal_column_font_count(CTreeItem *hTreeItem,int nColumn,int &nFontCount);
